...
class InvoiceManagement extends Component
{
    ...
    public function render()
    {
        ...
        // Get invoices from both the law firm directly and its lawyers
        $invoicesQuery = Invoice::where(function ($query) use ($lawFirmId) {
                // Invoices directly from the law firm
                $query->where('lawyer_id', $lawFirmId)
                    // OR invoices from lawyers belonging to this law firm
                    ->orWhereHas('lawyer', function ($subquery) use ($lawFirmId) {
                        $subquery->where('firm_id', $lawFirmId);
                    });
            })
            ->with(['client.clientProfile', 'legalCase', 'items', 'payments', 'lawyer', 'lawyer.lawyerProfile', 'lawyer.lawFirmLawyer']);
        ...
        if (empty($this->clients)) {
            // Get clients with completed consultations for this law firm or its lawyers
            $clientsQuery = User::whereHas('role', function($query) {
                    $query->where('name', 'client');
                })
                ->with('clientProfile') // Eager load client profiles
                ->where(function($query) use ($lawFirmId) {
                    // Clients with consultations with this law firm
                    $query->whereHas('clientConsultations', function($subquery) use ($lawFirmId) {
                        $subquery->where('lawyer_id', $lawFirmId)
                            ->where('status', 'completed');
                    })
                    // OR clients with consultations with lawyers of this law firm
                    ->orWhereHas('clientConsultations', function($subquery) use ($lawFirmId) {
                        $subquery->whereHas('lawyer', function($lawyerQuery) use ($lawFirmId) {
                            $lawyerQuery->where('firm_id', $lawFirmId);
                        })
                        ->where('status', 'completed');
                    });
                });
            ...
            if ($this->clients->isEmpty()) {
                $this->clients = User::whereHas('role', function($query) {
                    ...
                    ->with('clientProfile') // Eager load client profiles
                    ->where(function($query) use ($lawFirmId) {
                        // Clients with cases assigned to this law firm
                        $query->whereHas('cases', function($subquery) use ($lawFirmId) {
                            $subquery->where('lawyer_id', $lawFirmId);
                        })
                        // OR clients with cases assigned to lawyers of this law firm
                        ->orWhereHas('cases', function($subquery) use ($lawFirmId) {
                            $subquery->whereHas('lawyer', function($lawyerQuery) use ($lawFirmId) {
                                $lawyerQuery->where('firm_id', $lawFirmId);
                            });
                        });
        ...
        return view('livewire.law-firm.invoice-management', [
            'invoices' => $invoices,
    ...
    public function updatedSearchClient($query)
    {
        if (strlen($query) >= 2) {
            ...
            $consultationClients = (clone $clientsQuery)
                ->whereHas('clientConsultations', function($q) use ($lawFirmId) {
                    $q->where('lawyer_id', $lawFirmId)
                      ->where('status', 'completed');
                })
                ->take(10)
                ->get();
            ...
            if ($consultationClients->isEmpty()) {
                $caseClients = (clone $clientsQuery)
                    ->whereHas('cases', function($q) use ($lawFirmId) {
                        $q->where('lawyer_id', $lawFirmId);
                    })
                    ->take(10)
                    ->get();
    ...
    public function openInvoiceModal()
    {
        ...
        $clients = User::whereHas('role', function($query) {
            ...
            ->whereHas('clientConsultations', function($query) use ($lawFirmId) {
                $query->where('lawyer_id', $lawFirmId)
                    ->where('status', 'completed');
            })
            ->take(10)
        ...
        if ($clients->isEmpty()) {
            ...
            $clients = User::whereHas('role', function($query) {
                ...
                ->whereHas('cases', function($query) use ($lawFirmId) {
                    $query->where('lawyer_id', $lawFirmId);
                })
                ->take(10)
                ->get();
