        ...
        <div class="flex items-start space-x-4">
            ...
            <div class="flex-1 min-w-0 relative">
                    ...
                    <div class="mb-2 sm:mb-0">
                        <div class="flex items-center">
                            <h3 class="text-lg font-semibold text-gray-900 font-raleway">
                                {{ $lawyer->first_name }} {{ !empty($lawyer->last_name) ? substr($lawyer->last_name, 0, 1) . '.' : '' }}
                            </h3>
                            
                            <!-- Subscription tier badge -->
                            <span class="ml-5">
                                {!! renderSubscriptionBadge($lawyer->user) !!}
                            </span>
                        </div>
                        <div class="flex items-center mt-1 text-sm text-gray-500">
                            @php
                                $average = $lawyer->user->average_rating ?? 0;
                            ...
                            @if ($halfStar)
                                <svg class="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                                    <defs>
                                        <linearGradient id="halfStarDesktop{{ $lawyer->id }}" x1="0%" y1="0%" x2="100%" y2="0%">
                                            <stop offset="50%" stop-color="currentColor"></stop>
                                            <stop offset="50%" stop-color="#D1D5DB"></stop>
                                        </linearGradient>
                                    </defs>
                                    <path fill="url(#halfStarDesktop{{ $lawyer->id }})" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                                </svg>
                            @endif
                            ...
                            <span class="ml-1.5 mr-2">
                                {{ $lawyer->user->average_rating > 0 ? number_format($lawyer->user->average_rating, 1) : 'New' }}
                                @if($lawyer->user->rating_count > 0)
                                    ({{ $lawyer->user->rating_count }})
                                @endif
                            </span>
                        ...
                        @if($lawyer->show_office_address && $lawyer->office_address && $lawyer->lat && $lawyer->lng)
                            <div class="mt-1 text-sm text-gray-600">
                                <a href="https://www.openstreetmap.org/?mlat={{ $lawyer->lat }}&mlon={{ $lawyer->lng }}#map=16/{{ $lawyer->lat }}/{{ $lawyer->lng }}" 
                                   ...
                                   title="View on OpenStreetMap">
                                    ...
                                    {{ Str::limit($lawyer->office_address, 60) }}
                                </a>
                            </div>
                        @endif
                        
                        @if($type === 'firmLawyer' && $lawyer->lawFirm)
                            <div class="text-xs text-gray-500 mt-1">
                                Associated with <span class="font-medium text-gray-600">{{ $lawyer->lawFirm->firm_name }}</span>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <p class="mt-3 text-sm text-gray-700 font-open-sans leading-relaxed line-clamp-3 h-[4.5rem]">
                    @if($lawyer->about)
                        {{ Str::limit($lawyer->about, 200) }}
                    @elseif($lawyer->description)
                        {{ Str::limit($lawyer->description, 200) }}
                    @else
                ...
                @if($lawyer->services->count() > 0)
                    <div class="mt-4 flex flex-wrap gap-2 min-h-[2rem]">
                        @foreach($lawyer->services->take(5) as $service)
                        ...
                        @endforeach
                        @if($lawyer->services->count() > 5)
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800 border border-gray-200">
                                +{{ $lawyer->services->count() - 5 }} more
                            </span>
                        @endif
                    </div>
                @else
                ...
                <div class="mt-4 pt-4 border-t border-gray-200">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <div class="mb-2">
                                <span class="text-base font-semibold text-gray-900">₱{{ number_format($lawyer->min_budget) }} - ₱{{ number_format($lawyer->max_budget) }}</span>
                            ...
                            <div class="flex items-center space-x-2 mb-2">
                                @if($lawyer->offers_online_consultation)
                                    <span class="inline-flex items-center text-xs px-2.5 py-1 rounded-full bg-gray-100 text-gray-700 font-medium border border-gray-200">
                                        <svg class="h-3.5 w-3.5 mr-1 text-gray-500" fill="currentColor" viewBox="0 0 20 20"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z"></path><path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd"></path></svg>
                                        Online
                                    </span>
                                @endif
                                @if($lawyer->offers_inhouse_consultation)
                                    <span class="inline-flex items-center text-xs px-2.5 py-1 rounded-full bg-gray-100 text-gray-700 font-medium border border-gray-200">
                                        <svg class="h-3.5 w-3.5 mr-1 text-gray-500" fill="currentColor" viewBox="0 0 20 20"><path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z"></path></svg>
                                        In-House
                                    </span>
                                @endif
                            </div>
                        ...
                        <div class="flex space-x-2 mt-2 sm:mt-0">
                            <a href="{{ route('messages.chat', ['userId' => $lawyer->user->id]) }}" 
                               class="inline-flex items-center justify-center w-1/2 sm:w-auto px-5 py-2 border border-transparent text-sm font-medium rounded-full text-white bg-green-500 hover:bg-green-400 transition-colors duration-200">
                                Message
                            </a>
                            <a href="javascript:void(0);" 
                               class="inline-flex items-center justify-center w-1/2 sm:w-auto px-5 py-2 border border-transparent text-sm font-medium rounded-full text-white bg-green-500 hover:bg-green-400 transition-colors duration-200"
                               @click.prevent="(() => {
                                   const lawyerData = {{ json_encode($lawyer->load(['user.receivedRatings', 'user.activeSubscription.plan', 'lawFirm', 'services'])) }};
                                   console.log('Showing lawyer detail modal with data:', lawyerData);
                                   $dispatch('show-lawyer-detail', {
                                       lawyer: lawyerData, 
                                       type: '{{ $type === 'firmLawyer' ? 'lawyer' : 'lawyer' }}'
                                   });
                               })()">
                                View Profile
                            </a>
                    ...
                    @if($type === 'lawyer')
                        ...
                        @elseif($lawyer->law_firm_id && $lawyer->lawFirm)
                            <div class="mt-3 text-xs text-gray-500">
                                Associated with <span class="font-medium text-gray-700">{{ $lawyer->lawFirm->firm_name }}</span>
                            </div>
                        @endif
                    @endif
                </div>
