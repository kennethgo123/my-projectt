<div x-data="{ activeTab: 'overview' }" 
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">
                    {{ $case->case_number ? 'Criminal Case No. ' . $case->case_number . ': ' : '' }}{{ $case->title ?? 'N/A' }}
                </h1>
                <p class="text-sm text-gray-600 mt-1">
                    Filed: {{ $case->created_at ? $case->created_at->format('F d, Y') : 'N/A' }}
                </p>
                <div class="mt-2">
                    <a href="{{ route('law-firm.cases') }}" class="inline-flex items-center text-sm text-gray-600 hover:text-gray-800">
    ...
    <div>
                ...
                <div class="bg-white shadow-md rounded-lg p-6">
                    ...
                    <div class="space-y-3">
                        @if($tasks && $tasks->count() > 0)
                            @foreach($tasks->take(2) as $task) {{-- Displaying first 2 tasks as per image --}}
        ...
        <div x-show="activeTab === 'invoices'" x-cloak>
            <livewire:lawyer.case-invoices :case="$case" :readOnly="$isReadOnly" wire:key="case-invoices-{{ $case->id }}" />
        </div>
