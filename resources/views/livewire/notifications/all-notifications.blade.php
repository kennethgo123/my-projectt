    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div class="divide-y divide-gray-200">
            @forelse($notifications as $notification)
                @php
                    $isRead = $notification->read_at !== null || ($notification->is_read ?? false);
                    $notificationData = is_string($notification->data) ? json_decode($notification->data, true) : ($notification->data ?? []);
                    $notificationType = $notification->type ?? ($notificationData['type'] ?? '');
                    $title = $notification->title ?? $notificationData['title'] ?? 'Notification';
                    $content = $notification->content ?? $notificationData['message'] ?? '';
                @endphp
                <div 
                    ...
                    class="p-6 hover:bg-gray-50 {{ $isRead ? 'opacity-75' : 'border-l-4 border-indigo-500' }} {{ $notification->action_url ? 'cursor-pointer' : '' }}"
                ...
                >
                    <div class="flex items-start">
                        <div class="flex-shrink-0 pt-0.5">
                            @if($notificationType === 'consultation_request')
                            ...
                            @elseif($notificationType === 'case_started')
                            ...
                            @elseif($notificationType === 'new_message')
                                <span class="h-12 w-12 rounded-full bg-blue-100 flex items-center justify-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="ml-4 w-0 flex-1">
                            <div class="flex justify-between items-start">
                                <div class="flex-1">
                                    <div class="text-lg font-medium text-gray-900 {{ $notification->action_url ? 'hover:text-indigo-600' : '' }}">
                                        {{ $title }}
                                    </div>
