    ...
    <div class="bg-white shadow">
        <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            <div class="flex items-center">
                ...
                <h1 class="ml-4 text-3xl font-bold text-gray-900">{{ $lawFirm->firm_name }}</h1>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-start space-y-6 md:space-y-0 md:space-x-6 mb-10 bg-emerald-50 p-6 rounded-2xl border border-emerald-100 shadow-md">
            ...
            <div class="flex-shrink-0 flex justify-center">
                <div class="h-32 w-32 rounded-full bg-gradient-to-br from-emerald-100 to-emerald-200 flex items-center justify-center text-emerald-600 border-4 border-emerald-400 overflow-hidden shadow-lg">
                    @if($lawFirm->photo_path)
                        <img class="h-full w-full object-cover" src="{{ Storage::url($lawFirm->photo_path) }}" alt="Law firm logo">
                    @else
                        <svg class="h-16 w-16" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M3 21v-4a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4v4H3zM8 3h8a3 3 0 0 1 3 3v6H5V6a3 3 0 0 1 3-3z"/>
                        </svg>
                    @endif
                </div>
            </div>
            ...
            <div class="flex-1 space-y-4">
                <div>
                    <div class="flex items-center">
                        <h3 class="text-2xl font-bold font-raleway text-emerald-800 mb-2">{{ $lawFirm->firm_name }}</h3>
                    </div>
                    ...
                    <div class="flex items-center mt-1 text-sm text-gray-500">
                        ...
                        @if($lawFirm->user)
                            <div class="flex items-center">
                                @php
                                    $ratings = $lawFirm->user->receivedLawFirmRatings;
                    ...
                    @if($lawFirm->office_address && $lawFirm->lat && $lawFirm->lng)
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-emerald-200 hover:shadow-md transition-all duration-300">
                ...
                <div class="prose max-w-none text-gray-700 font-open-sans leading-relaxed">
                    {!! $lawFirm->pricing_description ?: 'Pricing information not available.' !!}
                    
                    <div class="mt-4 text-sm text-gray-600">
                        @if($lawFirm->min_budget && $lawFirm->max_budget)
                            <p>Professional Fee Range: ₱{{ number_format($lawFirm->min_budget) }} - ₱{{ number_format($lawFirm->max_budget) }}</p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-emerald-200 hover:shadow-md transition-all duration-300">
                ...
                <div class="prose max-w-none text-gray-700 font-open-sans leading-relaxed">
                    {!! $lawFirm->about ?: ($lawFirm->description ?? 'No description available.') !!}
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-emerald-200 hover:shadow-md transition-all duration-300">
                ...
                <div class="prose max-w-none text-gray-700 font-open-sans leading-relaxed">
                    {!! $lawFirm->experience ?: 'No professional experience information available.' !!}
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-emerald-200 hover:shadow-md transition-all duration-300">
                ...
                <div class="flex flex-wrap gap-2">
                    @if($lawFirm->services && $lawFirm->services->count() > 0)
            ...
            @if($lawyers->count() > 0)
                <div class="bg-white rounded-xl shadow-sm p-6 border border-emerald-200 hover:shadow-md transition-all duration-300">
                    <h4 class="text-lg font-semibold text-emerald-800 mb-4 font-raleway flex items-center">
                        ...
                        LAWYERS AT {{ strtoupper($lawFirm->firm_name) }}
                    </h4>
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="flex-shrink-0">
                                        @if($lawyer->photo_path)
                                            <img class="h-12 w-12 rounded-full object-cover" src="{{ Storage::url($lawyer->photo_path) }}" alt="{{ $lawyer->first_name }} {{ $lawyer->last_name }}">
                                        @elseif($lawyer->user && $lawyer->user->profile_photo_path)
                                            <img class="h-12 w-12 rounded-full object-cover" src="{{ Storage::url($lawyer->user->profile_photo_path) }}" alt="{{ $lawyer->first_name }} {{ $lawyer->last_name }}">
                                        @else
                                        ...
                                        @if($lawyer->services && $lawyer->services->count() > 0)
                                            <div class="mt-1">
                                                <div class="flex flex-wrap gap-1">
                                                    @foreach($lawyer->services->take(2) as $service)
                                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-100 text-emerald-800">
                                                            {{ $service->name }}
                                                        </span>
                                                    @endforeach
                                                    @if($lawyer->services->count() > 2)
                                                        <span class="text-xs text-gray-500">+{{ $lawyer->services->count() - 2 }} more</span>
                                                    @endif
                                                </div>
                                            </div>
                                        @endif

                                        {{-- Rating for individual lawyer --}}
                                        @if($lawyer->user && $lawyer->user->receivedRatings->count() > 0)
                                            @php
                                                $avgRating = $lawyer->user->receivedRatings->avg('rating');
                                                $ratingCount = $lawyer->user->receivedRatings->count();
                                            @endphp
                                            <div class="flex items-center mt-1">
                                                <div class="flex items-center">
                                                    @for($i = 1; $i <= 5; $i++)
                                                        <svg class="w-3 h-3 {{ $i <= round($avgRating) ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                                                        </svg>
                                                    @endfor
                                                </div>
                                                <span class="ml-1 text-xs text-gray-500">{{ number_format($avgRating, 1) }} ({{ $ratingCount }})</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                    ...
                                    @auth
                                        @if(auth()->user()->role->name === 'client')
                                            <a href="{{ route('messages.chat', ['userId' => $lawyer->user->id]) }}" 
                                               class="flex-1 inline-flex justify-center items-center px-3 py-1.5 border border-gray-300 text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50">
                                                Message
                                            </a>
                                            <a href="{{ route('lawyer.profile', ['user' => $lawyer->user->id]) }}" 
                                               class="flex-1 inline-flex justify-center items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded text-white bg-emerald-600 hover:bg-emerald-700">
                                                View Profile
                                            </a>
                                        @endif
                                    @else
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-emerald-200 hover:shadow-md transition-all duration-300">
                ...
                <div>
                    @if($lawFirm->user)
                        @php
                            $reviews = $lawFirm->user->receivedLawFirmRatings()->where('is_visible', true)->with('client.clientProfile')->latest()->get();
                        @endphp
